function results = AFOSM_SVR_EX1_DI(case_id)
% AFOSM_SVR_EX1_DI
% Single main entry for the modular AFOSM-SVR paper-figure workflow.
%
% This main program implements:
% 1) AFOSM-SVR hybrid iterations (matching steps shown in the images)
% 2) Classic AFOSM (HL-RF style) baseline
% 3) GA (or fallback) design-point benchmark
% 4) Multi-run statistics and publication-style figures
% 5) SVR backend comparison (UQLab SVR vs Matlab fitrsvm-SVR)

clc;
close all;
format long g;

% Keep all helper functions in this folder on MATLAB path.
...
%% Main settings and problem definition
cfg = default_config();
...
prob = build_problem(cfg.case_id);
...
fprintf("==== AFOSM-SVR paper-style run ====\n");
fprintf("Case ID: %d, dimension: %d\n", cfg.case_id, prob.Nx);
...
beta_ref = -norminv(clamp01(pf_ref));
...
while accepted_runs < cfg.n_runs
    ...
    fprintf("  %s[%s]: u*=[%.4f, %.4f], beta=%.4f, Pf_IS=%.6e, CoV_IS=%.4f, Ncall=%d", ...
        labels.afosm_beta_equation, upper(svm_i.svr_backend), svm_i.u_final(1), svm_i.u_final(2), svm_i.beta_final, svm_i.pf_is, svm_i.cov_is, svm_i.n_true_calls);
    ...
    for ib = 2:numel(svr_backends)
        ...
        fprintf("  %s[%s]: u*=[%.4f, %.4f], beta=%.4f, Pf_IS=%.6e, CoV_IS=%.4f, Ncall=%d", ...
            labels.afosm_beta_equation, upper(svm_cmp.svr_backend), svm_cmp.u_final(1), svm_cmp.u_final(2), ...
    ...
    fprintf("  %s[%s]: u*=[%.4f, %.4f], beta=%.4f, Pf_IS=%.6e, CoV_IS=%.4f, Ncall=%d, err=%.3f%%\n", ...
        labels.afosm_direct_iteration, upper(af_backend), afosm_i.u_final(1), afosm_i.u_final(2), afosm_i.beta, afosm_i.pf_is, afosm_i.cov_is, afosm_i.n_true_calls, err_afosm);
    ...
    fprintf("  GA       : u*=[%.4f, %.4f], beta=%.4f, Pf_IS=%.6e, CoV_IS=%.4f, Ncall=%d, err=%.3f%%\n\n", ...
        ga_i.u_final(1), ga_i.u_final(2), ga_i.beta, ga_i.pf_is, ga_i.cov_is, ga_i.n_true_calls, err_ga);
end
...
% Persist all outputs in one MAT file for downstream plotting/reporting.
save(fullfile(out_dir, "AFOSM_SVR_results.mat"), "results", "-v7.3");
fprintf("Saved data to: %s\n", fullfile(out_dir, "AFOSM_SVR_results.mat"));
