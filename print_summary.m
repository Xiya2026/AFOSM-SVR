...
% print_summary: Helper function in the AFOSM-SVR modular workflow.
...
if isfield(summary, "afosm_backend")
...
fprintf("%s[%s]: u*_mean = [%.4f, %.4f]\n", labels.afosm_beta_equation, char(upper(string(primary_backend))), ...
    summary.svm.u_mean(1), summary.svm.u_mean(2));
...
fprintf("%s: u*_mean = [%.4f, %.4f]\n", labels.afosm_direct_iteration, ...
    summary.afosm.u_mean(1), summary.afosm.u_mean(2));
fprintf("           Pf_IS(mean) = %.6e, CoV_IS(mean) = %.4f\n", ...
...
fprintf("GA       : u*_mean = [%.4f, %.4f]\n", ...
    summary.ga.u_mean(1), summary.ga.u_mean(2));
fprintf("           Pf_IS(mean) = %.6e, CoV_IS(mean) = %.4f\n", ...
