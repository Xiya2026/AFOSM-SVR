function [beta, n_eval, success] = solve_beta_on_direction(prob, alpha, beta0, cfg)
...
try
    beta = fzero(@(b) eval_beta_cached(b), [bL, bU]);
catch
    beta = 0.5 * (bL + bU);
end
if ~isfinite(beta)
...
success = abs(eval_beta_cached(beta)) <= cfg.root_accept_tol;
end
