...
% summarize_results: Helper function in the AFOSM-SVR modular workflow.
n = num_runs(svm_runs);
svm_pf = zeros(n, 1);
svm_beta = zeros(n, 1);
svm_err = zeros(n, 1);
...
for i = 1:n
    ...
    svm_u(i, :) = first_two(svm_i.u_final);
...
if n >= 1 && isfield(get_run(afosm_runs, 1), "afosm_backend")
    summary.afosm_backend = get_run(afosm_runs, 1).afosm_backend;
else
    summary.afosm_backend = "unknown";
end
summary.afosm_error = "";
for i = 1:n
    af_i = get_run(afosm_runs, i);
